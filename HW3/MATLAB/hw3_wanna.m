...
% square_points = [RRP.joint_pose_list(:,1,3)'; square_points];
...
for side = 1:num_polygon_sides
    ...
    eef_vel_update = cubic_path(square_points(side,:), square_points(end_point,:)); % give vd values for 400 time steps/ one side of a square
        
    % curr joint poses from start of path: draw one side of square
%     q_rev_init = RRP.table(:,1);
%     q_pris_init = RRP.table(:,2);
    
    for t = 1:length(eef_vel_update)  
        ...
        % TODONE: Also eef_vel_update contains n time steps of vel updates. Need to perform this iteratively
        ...
        % TODONE: Use joint_pose_update values to update joint vars
